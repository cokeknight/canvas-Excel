        ...
        co.clip();
            ...
            for (var i=0, j=0; i<this.data.length; i++, j++) {
                ...
                */
                ...
                this.DrawLine(this.data[i],prop['chart.colors'][j],fill,this.GetLineWidth(j), tickmarks,i);
    
                this.context.stroke();
            }
    
        /**
        * If the line is filled re-stroke the lines  描绘线
        */
        if (prop['chart.filled'] && prop['chart.filled.accumulative'] && !prop['chart.curvy']) {
            ...
            for (var i=0; i<this.coords2.length; ++i) {
                ...
                for (var j=0; j<this.coords2[i].length; ++j) {
					...
					co.closePath();
					if(typeof fill==='object' && prop['chart.grouping'] !== "stacked"){
					var yPosition = this.properties['chart.xaxispos'] == 'center' ? ((this.canvas.height - this.gutterTop - this.gutterBottom) / 2) + this.gutterTop : this.canvas.height - this.gutterBottom;
							...
							if(this.coords2[i][j+1]!==undefined){
								co.beginPath();
								co.lineTo(this.coords2[i][j][0], this.coords2[i][j][1]);
								co.lineTo(this.coords2[i][j][0], yPosition);
								co.lineTo(this.coords2[i][j+1][0], yPosition);
								co.lineTo(this.coords2[i][j+1][0], this.coords2[i][j+1][1]);
								co.fillStyle=fill[j]||fill[j%fill.length];
								co.fill();
								co.closePath();
							}
					}
